 ...
 */
class SearchUserByEmailQuery extends SelectQuery<SearchUserByEmailParams, SearchUserRecord> {
		...
		this.prepared = {
			user: `
				...
				where user.${user.email} = ?
					...
					and (
						user.${user.id} = ?
						or priv.${priv.discoverableByEmail} = 1
	...
	private compileForUser(email: string, userId: number) : string {
	...
	compile({ email, searchAs }: SearchUserByEmailParams) : string {
		if (searchAs.isAdmin) {
			return format(this.prepared.admin, [ searchAs.userId, email ]);
		}

		return this.compileForUser(email, searchAs.userId);
	}
...
export const searchUserByEmail = new SearchUserByEmailQuery();
